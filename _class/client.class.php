...
class client {
  ...
  public static function getMagasinsSecteur() {
    global $db;
    ...
    // Vérifier si c'est un administrateur (peut voir tous les magasins)
    if( securite::isAdmin() ) {
      $db->execute("SELECT id_as400, enseigne, raison_sociale FROM ref_client WHERE id_as400 IS NOT NULL AND deleted = 0 AND actif = 1 ORDER BY enseigne");
    }
    // Sinon, récupérer les magasins du même secteur
    else {
      // Vérifier que l'utilisateur a un secteur défini
      if( empty($user['secteur']) ) {
        die(json_encode([]));
      }
      // Pour l'instant, on retourne tous les magasins car on n'a pas de relation directe secteur/magasin
      // TODO: Implémenter la logique de filtrage par secteur si nécessaire
      $db->execute("SELECT id_as400, enseigne, raison_sociale FROM ref_client WHERE id_as400 IS NOT NULL AND deleted = 0 AND actif = 1 ORDER BY enseigne");
    }
    
    $magasins = [];
    while( $r = $db->assoc() ) {
      $nom = !empty($r['enseigne']) ? $r['enseigne'] : $r['raison_sociale'];
      $magasins[] = [
        'id_as400' => $r['id_as400'],
        'nom' => $nom
      ];
