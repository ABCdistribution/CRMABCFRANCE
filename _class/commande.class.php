...
public static function getJuvaCmd($limit = 15, $id = null, $full = true) {
    global $db;
    $w = "";
    if ($id > 0) {
        $w = " WHERE id = " . intval($id);
    }

    $db->execute("SELECT * FROM juva_commande $w ORDER BY id DESC LIMIT $limit");
    $cmds = $db->getArray();

    foreach ($cmds as $k => $e) {
        if (!$full) {
            $cmds[$k]['client'] = client::getByCode($e['id_as400'])['enseigne'];
            $cmds[$k]['user'] = user::getNameFromLogin($e['utilisateur']);
            $cmds[$k]['daterealisation'] = core::dateOutput($e['daterealisation']);
            $cmds[$k]['datelivraison'] = core::dateOutput($e['datelivraison']);
            $cmds[$k]['total'] = core::n($e['total']);
        }

        $cmds[$k]['produits'] = [];

        $db->execute("SELECT * FROM juva_commandeligne WHERE commande = '" . $db->escape($e['code']) . "'");
        $produits = $db->getArray();

        foreach ($produits as $r) {
            $db->execute("SELECT * FROM juva_produit WHERE idoriginal = '" . $db->escape($r['produit']) . "' LIMIT 1");
            if (!$db->num()) continue;
            $a = $db->assoc();

            if ($full) {
                $cmds[$k]['produits'][$r['produit']] = $a;
            } else {
                
                $cmds[$k]['produits'][$r['produit']]['pu'] = core::n($a['prix']);
                $pu = floatval($a['prix']);
                $qte = intval($r['quantite']);
                $pcb = intval($a['pcb']) ?: 1;
                $pt = $pu * $qte * $pcb;

                $cmds[$k]['produits'][$r['produit']]['libelle'] = $a['libelle'] ?? '';
                $cmds[$k]['produits'][$r['produit']]['pu'] = core::n($pu);
                $cmds[$k]['produits'][$r['produit']]['pt'] = core::n($pt);
                $cmds[$k]['produits'][$r['produit']]['pcb'] = $pcb;
                $cmds[$k]['produits'][$r['produit']]['qte'] = $qte;
            }

            $cmds[$k]['produits'][$r['produit']]['qte'] = $r['quantite'];
        }
    }
  ...
  public static function getHistoriqueCommande() {
    ...
    $db->execute("SELECT * FROM juva_commande WHERE  utilisateur = '".$db->escape($user['login'])."' ORDER BY id DESC LIMIT 100");
    if ($db->num()) {
        $juvaCommandes = $db->getArray();
        foreach ($juvaCommandes as $e) {
            $tmp = [
                "id" => $e['id'],
                "c" => client::getByCode($e['id_as400'])['enseigne'],
                 "d"  => core::dateOutput($e['daterealisation']),
                 "f"  => core::dateFrom($e['daterealisation']),
                 "ts" => $e['daterealisation'], 
                "t"  => core::n($e['total']),
                "juva" => true, // Indique que c'est une commande Juva
            ];
            $c[] = $tmp;
        }
    } 
    usort($c, function($a, $b) {
    return strtotime($b['f']) - strtotime($a['f']);
});
...
public static function getHistoriqueCommandeDetails() {
    ...
    // Est-ce une commande Juva ?
    $juva = self::getJuva($id);

    if ($juva) {
        $cmds = self::getJuvaCmd(1, $id, false);
    } else {
    ...
    // Pour les commandes non Juva, formatage APK
    if (!$juva) {
