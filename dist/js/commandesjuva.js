$(document).ready(function() {
  ...
  $(document).on('click.vTable','#vTable tbody tr',function() {
    window.open(_global.app_url+'CommandesJuva/'+$(this).attr('data-id') )
  })
...
function loadTableCommande() {
  ...
  loader(true,table);
  ajax({
    methode:'commande::searchBoardJuva',
...
function exportJuvaExcel() {
  const from = $("input[name=from]").val();
  const to = $("input[name=to]").val();
  const str = $("#searchField").val().trim();
  ...
  add('methode', 'commande::exportBoardJuva');
  add('from', from);
  add('to', to);
  add('str', str);
