...
$(document).ready(function() {
	...
	$(window).bind('beforeunload', function(){
		if ( window.location.href.toLowerCase().indexOf('retours') === -1 ) {
			loader();
		}
	});
})
...
let curImg = null;
let pvScale = 1, pvRot = 0, pvX = 0, pvY = 0;
function pvApply() {
	let img = $("#photoViewer img");
	if( !img.length ) return;
	img.css('transform','translate('+pvX+'px,'+pvY+'px) rotate('+pvRot+'deg) scale('+pvScale+')');
	img.toggleClass('pv-zoomed', (pvScale !== 1 || pvRot !== 0));
}
function pvReset() {
	pvScale = 1; pvRot = 0; pvX = 0; pvY = 0;
	pvApply();
}
$(document).ready(function() {
	...
	$(document).on('click.pvctrl','#photoViewerControls .pv-btn',function(e) {
		e.preventDefault();
		e.stopPropagation();
		let act = $(this).data('act');
		if( act === 'zoom-in' ) pvScale = Math.min(pvScale + 0.25, 8);
		else if( act === 'zoom-out' ) pvScale = Math.max(pvScale - 0.25, 0.25);
		else if( act === 'rot-left' ) pvRot -= 90;
		else if( act === 'rot-right' ) pvRot += 90;
		else if( act === 'reset' ) { pvReset(); return false; }
		pvApply();
		return false;
	})
	$(document).on('wheel.pvzoom','#photoViewer',function(e) {
		if( !$("#photoViewer").is(':visible') ) return;
		e.preventDefault();
		let delta = e.originalEvent.deltaY < 0 ? 0.2 : -0.2;
		pvScale = Math.min(Math.max(pvScale + delta, 0.25), 8);
		pvApply();
	})
	let pvDrag = false, pvSX = 0, pvSY = 0, pvMoved = false;
	$(document).on('mousedown.pvpan','#photoViewer img',function(e) {
		if( pvScale === 1 && pvRot === 0 ) return;
		e.preventDefault();
		pvDrag = true;
		pvMoved = false;
		pvSX = e.clientX - pvX;
		pvSY = e.clientY - pvY;
		$(this).addClass('pv-dragging');
	})
	$(document).on('mousemove.pvpan',function(e) {
		if( !pvDrag ) return;
		pvMoved = true;
		pvX = e.clientX - pvSX;
		pvY = e.clientY - pvSY;
	...
	$(document).on('mouseup.pvpan',function() {
		if( !pvDrag ) return;
		pvDrag = false;
		$("#photoViewer img").removeClass('pv-dragging');
	})
	$(document).on('click.pvimg','#photoViewer img',function(e) {
		if( pvMoved || pvScale !== 1 || pvRot !== 0 ) {
			e.stopPropagation();
			pvMoved = false;
		}
	})
