...
export default function JoursOFF() {
  const joursOFF = [
    {
      id: 1,
    ...
    },
    {
      ...
      statut: "En attente",
      duree: "0.5 jour",
      motif: "Démarches personnelles",
      demande: "16/01/2024",
      approbateur: "Sophie Bernard"
    },
    {
      id: 3,
  ...
  const getStatutColor = (statut: string) => {
    const colors = {
      'Approuvé': { bg: '#dcfce7', color: '#16a34a' },
  ...
  const getTypeColor = (type: string) => {
    const colors = {
      'Congés payés': { bg: '#dbeafe', color: '#2563eb' },
      'RTT': { bg: '#dcfce7', color: '#16a34a' },
      'Formation': { bg: '#f3e8ff', color: '#9333ea' },
      'Maladie': { bg: '#fee2e2', color: '#dc2626' }
    }
  ...
  return (
    <div className="page-container">
      ...
      <div className="page-header">
        <div>
          <h1 style={{ fontSize: '2rem', fontWeight: 'bold', color: '#1f2937', margin: 0 }}>Jours OFF</h1>
          <p style={{ color: '#6b7280', margin: '0.5rem 0 0 0' }}>Gérez les demandes de congés et jours de repos</p>
        </div>
      ...
      <Card style={{ 
        ...
        <CardHeader style={{ 
          ...
          <CardTitle style={{ fontSize: '1.125rem', fontWeight: '600', color: '#1f2937', margin: 0 }}>
            Liste des demandes de jours OFF
          </CardTitle>
        </CardHeader>
        <CardContent style={{ padding: 0 }}>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {joursOFF.map((demande, index) => {
                  const statutColors = getStatutColor(demande.statut)
                  const typeColors = getTypeColor(demande.type)
                  return (
                    <tr key={demande.id} style={{ borderBottom: index < joursOFF.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
                      <td style={{ padding: '0.75rem 1rem', fontSize: '0.875rem', color: '#1f2937' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          ...
                          <div>
                            <div style={{ fontWeight: '500' }}>{demande.utilisateur}</div>
                            <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>Demandé le {demande.demande}</div>
                          </div>
                        </div>
                      </td>
