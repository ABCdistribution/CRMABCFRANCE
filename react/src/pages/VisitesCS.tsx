...
export default function VisitesCS() {
  const visitesCS = [
    {
      id: 1,
    ...
    },
    {
      ...
      statut: "Réalisée",
      type: "Suivi client",
      objectif: "Négociation contrat",
      adresse: "45 Avenue des Champs, 69000 Lyon",
      resultat: "Contrat signé"
    },
    {
      id: 3,
  ...
  const getTypeColor = (type: string) => {
    const colors = {
      'Visite commerciale': { bg: '#dbeafe', color: '#2563eb' },
      'Suivi client': { bg: '#dcfce7', color: '#16a34a' },
      'Formation': { bg: '#f3e8ff', color: '#9333ea' },
      'Rendez-vous': { bg: '#fef3c7', color: '#d97706' }
    }
  ...
  const getResultatColor = (resultat: string) => {
    const colors = {
      'Contrat signé': { bg: '#dcfce7', color: '#16a34a' },
      'En attente': { bg: '#fef3c7', color: '#d97706' },
      'En cours': { bg: '#dbeafe', color: '#2563eb' },
      'Reporté': { bg: '#fee2e2', color: '#dc2626' }
    }
    return colors[resultat as keyof typeof colors] || { bg: '#f3f4f6', color: '#6b7280' }
  }
  ...
  return (
    <div className="page-container">
      ...
      <div className="page-header">
        <div>
          <h1 style={{ fontSize: '2rem', fontWeight: 'bold', color: '#1f2937', margin: 0 }}>Visites commerciales</h1>
          <p style={{ color: '#6b7280', margin: '0.5rem 0 0 0' }}>Gérez les visites commerciales et le suivi client</p>
        </div>
        ...
        <CardContent style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
            <div style={{ position: 'relative', flex: 1 }}>
              ...
              <input 
                type="text" 
                placeholder="Rechercher une visite commerciale..."
                style={{ 
                  width: '100%', 
      ...
      {/* Tableau des visites commerciales */}
      <Card style={{ 
        border: '2px solid #e5e7eb',
        backgroundColor: 'white',
        boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)',
        ...
        <CardHeader style={{ 
          ...
          <CardTitle style={{ fontSize: '1.125rem', fontWeight: '600', color: '#1f2937', margin: 0 }}>
            Liste des visites commerciales
          </CardTitle>
        </CardHeader>
        <CardContent style={{ padding: 0 }}>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {visitesCS.map((visite, index) => {
                  const statutColors = getStatutColor(visite.statut)
                  const typeColors = getTypeColor(visite.type)
                  const resultatColors = getResultatColor(visite.resultat)
                  return (
                    <tr key={visite.id} style={{ borderBottom: index < visitesCS.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
                      <td style={{ padding: '0.75rem 1rem', fontSize: '0.875rem', color: '#1f2937' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <div style={{ 
                            ...
                            <Briefcase style={{ width: '1rem', height: '1rem', color: '#6b7280' }} />
                          </div>
                          <div>
                            <div style={{ fontWeight: '500' }}>{visite.client}</div>
                            <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>{visite.adresse}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td style={{ padding: '0.75rem 1rem', fontSize: '0.875rem' }}>
                        <span style={{ 
                          ...
                          fontSize: '0.75rem',
                          fontWeight: '500',
                          backgroundColor: resultatColors.bg,
                          color: resultatColors.color
                        }}>
                          {visite.resultat}
                        </span>
                      </td>
